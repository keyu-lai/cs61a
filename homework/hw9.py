...
class Mobile:
    ...
    def is_balanced(self):
        ...
        "*** YOUR CODE HERE ***"
        tmp = True
        if type(self.left.contents) == Mobile:
            tmp = tmp and self.left.contents.is_balanced()
        if type(self.right.contents) == Mobile:
            tmp = tmp and self.right.contents.is_balanced()
        return tmp and (self.left.torque == self.right.torque)
...
class Stream:
    ...
    def __iter__(self):
        ...
        tmp = self
        yield tmp.first
        while tmp.rest is not Stream.empty:
            tmp = tmp.rest
            yield tmp.first
    ...
    def __getitem__(self, k):
        ...
        tmp = self
        while k > 0:
            if tmp.rest is not Stream.empty:
                tmp, k = tmp.rest, k-1
            else:
...
def unique(s):
    ...
    def res():
        nonlocal buf, tmp
        tmp = tmp.rest
        if tmp is not Stream.empty:
            if tmp.first not in buf:
                buf.append(tmp.first)
                return Stream(tmp.first, res)
            else:
                return res()
        else:
            return Stream.empty
...
def rle(s, max_run_length=10):
    ...
    def res():
        nonlocal tmp
        if tmp is Stream.empty:
            return Stream.empty
        i = 0
        e = tmp.first
        while tmp is not Stream.empty and tmp.first == e and i < max_run_length:
            tmp = tmp.rest
            i += 1
        return Stream((i, e), res)
    tmp = s
    return res()
