...
def has_prefix(s, prefix):
	...
	if prefix == empty:
		...
		return False
	return first(prefix) == first(s) and has_prefix(rest(s), rest(prefix))
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, try_password):
    	if len(password_history) == 3:
    		return "Your account is locked. Attempts: {0}".format(password_history)
    		...
    		password_history.append(try_password)
