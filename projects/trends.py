...
def analyze_tweet_sentiment(tweet):
    ...
    for i in tweet_words(tweet):
        ...
        if has_sentiment(sentiment):
            sum, count = sum+sentiment_value(sentiment), count+1
    if count > 0:
...
def find_state_center(polygons):
    ...
    for item in polygons:
        lat, lon, area = find_centroid(item)
        x, y, A = x+lat*area, y+lon*area, A+area 
    return make_position(x/A, y/A)
...
def average_sentiments(tweets_by_state):
    ...
    for key in tweets_by_state.keys():
        ...
        for tweet in tweets_by_state[key]:
            ...
            if has_sentiment(sentiment):
                tmp, count = tmp+sentiment_value(sentiment), count+1
        if count > 0:
            result[key] = tmp/count
    return result
##########################
