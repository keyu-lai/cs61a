...
def max_scoring_num_rolls(dice=six_sided):
    ...
    ave_max = 0
    while n <= 10:
        tmp = make_averaged(roll_dice)(n, dice)
        if tmp > ave_max:
            n_max = n
            ave_max = tmp
        n += 1
...
def run_experiments():
    ...
    if False: # Change to True to test swap_strategy
        # print('final_strategy win rate:', sum / 50)
        print('swap_strategy win rate:', average_win_rate(swap_strategy))

    if True: # Change to True to test final_strategy
        print('final_strategy win rate:', make_averaged(average_win_rate, 1000)(final_strategy))
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=5):
    ...
    """
    "*** YOUR CODE HERE ***"
    tmp = abs(opponent_score // 10 - opponent_score % 10) + 1
    if tmp >= margin:
        return 0
    else:
        return num_rolls
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=5):
    ...
    """
    "*** YOUR CODE HERE ***"
    tmp_score = abs(opponent_score % 10 - opponent_score // 10) + 1 + score
    if opponent_score == tmp_score * 2:
        return 0
    elif tmp_score == opponent_score * 2:
        return num_rolls
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    "*** YOUR CODE HERE ***"
    # Determine wheather a four_sized is used
    def is_four(score, opponent_score):
        return (score + opponent_score) % 7 == 0

    # Trigger a beneficial swine swap by trying to score only one point
    # when margin = 10, bacon rule is forbidden
    if opponent_score == (score + 1) * 2:
        return swap_strategy(score, opponent_score, 10, 10)

    # leave your opponent with four_sided dice more often
    # average gained point with a four_sized is about 3.6, gaining more points in this turn
    bacon_points = abs(opponent_score // 10 - opponent_score % 10) + 1
    if is_four(score + bacon_points, opponent_score) and bacon_points >= 4:
        return swap_strategy(score, opponent_score, 10, 0)

    # when losing more than 30 points, try to trigger a beneficial swin swap by rolling one(16.67% or 25%)
    if opponent_score % 2 == 0 and opponent_score > 60:
        tmp = opponent_score / 2 - score
        if is_four(score, opponent_score):
            if tmp > 0 and tmp <= 4:
                return swap_strategy(score, opponent_score, 10, 1)
        else:
            if tmp > 0 and tmp <= 6:
                return swap_strategy(score, opponent_score, 10, 1)
    ...
    # expecting gained points of rolling 3 with a four_sided is 4.3
    # expecting gained points of rolling 5 with a six_sided is 8.6
    extra = 0
    tmp = score - opponent_score
    if tmp > -20 and tmp < 0:
        extra = 1
    elif tmp <= -20:
        extra = 2
        ...
        extra = -1
    if is_four(score, opponent_score):
        return swap_strategy(score, opponent_score, 4, 3 + extra)
    else:
        return swap_strategy(score, opponent_score, 8, 5 + extra)
    # bacon_points = abs(opponent_score // 10 - opponent_score % 10) + 1
