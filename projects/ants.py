...
class ThrowerAnt(Ant):
	...
	def nearest_bee(self, hive):
		...
		"""
		"*** YOUR CODE HERE ***"
		itr, num = self.place, 0 
		while itr:
			if (itr.entrance == hive) or (num >= self.max_range) or ((self.min_range <= num < self.max_range) and itr.bees):
				break
			itr, num = itr.entrance, num+1
		if self.min_range <= num < self.max_range:
			return random_or_none(itr.bees)
		else:
			return random_or_none(None)
...
class QueenPlace:
	...
	def __init__(self, colony_queen, ant_queen):
		"*** YOUR CODE HERE ***"
		self.place1 = colony_queen
		self.place2 = ant_queen

	@property
	def bees(self):
		"*** YOUR CODE HERE ***"
		return self.place1.bees + self.place2.bees
...
class QueenAnt(ScubaThrower):  # You should change this line
	...
	def __init__(self):
		...
		QueenAnt.count += 1
		self.imposter = (QueenAnt.count>1)
		self.ants_doubled = [self]
	...
	def action(self, colony):
		...
		def helper(place):
			if place.ant != None:
				if place.ant not in self.ants_doubled:
					place.ant.damage *= 2
					self.ants_doubled.append(place.ant)
				if place.ant.container:
					if place.ant.ant:
						if place.ant.ant not in self.ants_doubled:
							place.ant.ant.damage *= 2
							self.ants_doubled.append(place.ant.ant)
		place = self.place
		while place.exit:
			place = place.exit
			helper(place)
